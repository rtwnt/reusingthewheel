...
Ligature is similar to what is called construct state, to the point of the ligature in Miskito having been originally misleadingly called "construct state" - but there are differences: [Semitic/Afro-asiatic construct state is limited to possessive constructions][4]. Nevertheless, I think it would be interesting to include construct state (or an adposition that would grammaticalize to a construct state suffix) in the proto-language and explore its evolution throughtout the whole language family and have it interact with the ligature in various ways.
